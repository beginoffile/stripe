...
func (app *application) WsEndPoint(w http.ResponseWriter, r *http.Request) {
	...
	app.infoLog.Println(fmt.Sprintf("client connect to %s", r.RemoteAddr))
...
func (app *application) ListenForWS(conn *WebSocketConnection) {
	defer func() {
		r := recover()
		if r != nil {
			app.errorLog.Println("ERRROR:", fmt.Sprintf("%v", r))
		}
	}()
...
func (app *application) ListenToWsChanner() {
	var response WsJsonResponse
	for {
		e := <-wsChan
		switch e.Action {
		case "deleteUser":
			...
			response.Message = "Your count has been deleted"
