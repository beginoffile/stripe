...
// Invoice describes the JSON payload sent to microservice
type Invoice struct {
	ID        int       `json:"id"`
...
func (app *application) CreateCustomerAndSubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	if okay {
		...
		inv := Invoice{
			ID:        orderID,
			Amount:    order.Amount,
			Product:   "Bronze Plan montly subscription",
...
func (app *application) AllSales(w http.ResponseWriter, r *http.Request) {
	...
	allSales, lastPage, totalRecors, err := app.DB.GetAllOrdersPaginated(payload.PageSize, payload.CurrentPage)
	if err != nil {
		app.badRequest(w, r, err)
		return
	}
...
func (app *application) AllSubscriptions(w http.ResponseWriter, r *http.Request) {
	...
	allSales, lastPage, totalRecors, err := app.DB.GetAllSubscriptionsPaginated(payload.PageSize, payload.CurrentPage)
	if err != nil {
		app.badRequest(w, r, err)
		return
	}
...
func (app *application) GetSale(w http.ResponseWriter, r *http.Request) {
	...
	order, err := app.DB.GetOderByID(orderID)
	if err != nil {
		app.badRequest(w, r, err)
		return
	}
...
func (app *application) RefundCharge(w http.ResponseWriter, r *http.Request) {
	...
	card := cards.Card{
		...
		Currrency: chargeToRefund.Currency,
	}
...
func (app *application) CancelSubscription(w http.ResponseWriter, r *http.Request) {
	...
	card := cards.Card{
		Secret:    app.config.stripe.secret,
		Key:       app.config.stripe.key,
		Currrency: subToCancel.Currency,
	}

	err = card.CancelSubscription(subToCancel.PaymenIntent)
	if err != nil {
		app.badRequest(w, r, err)
		return
	}
